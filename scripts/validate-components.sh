...
# Components that are allowed but not in the upstream builder-config.yaml
...
for exception in "${EXCEPTION_COMPONENTS[@]}"; do
  valid_components+=$'\n'"$exception"
done
...
if [[ -n "$invalid_components" ]]; then
  ...
  echo "Or be included in the exceptions list"
